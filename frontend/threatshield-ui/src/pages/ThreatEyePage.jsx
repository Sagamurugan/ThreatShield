...
function ThreatEyePage() {
  const [urlToScan, setUrlToScan] = useState('');
  // CHANGE 1: State is now an array to hold history, and renamed for clarity
  ...
  const handleScan = async () => {
    ...
    try {
        const response = await axios.post('http://localhost:3001/api/scan/url', { url: urlToScan });
        // CHANGE 2: Add the new result to the BEGINNING of the history array
        setScanHistory(prevHistory => [response.data, ...prevHistory]);
    } catch (error) {
        console.error("Error scanning URL:", error);
        const errorResult = { error: "Scan failed. Ensure backend services are running.", url: urlToScan };
        // Also add errors to the history to see what failed
        setScanHistory(prevHistory => [errorResult, ...prevHistory]);
    }

    setIsLoading(false);
    setUrlToScan(''); // Bonus: Clear the input field after scanning
  };

  // Bonus: Allow pressing Enter to scan
        ...
        <main className="content-card">
          <div className="url-scanner-input">
            ...
            <input 
              ...
              value={urlToScan}
              onChange={(e) => setUrlToScan(e.target.value)}
              onKeyDown={handleKeyDown} // Bonus: Added Enter key functionality
            />
            <button className="scan-button" onClick={handleScan} disabled={isLoading}>
              {isLoading ? <FaSpinner className="spinner" /> : 'Scan'}
            </button>
          ...
          {/* CHANGE 3: Check if history array has items and then map over it */}
          {scanHistory.length > 0 && (
             <div className="results-container">
                <h3 className="results-title">Scan History</h3>
                <ul className="results-list">
                    {scanHistory.map((result, index) => (
                        <ScanResult key={index} result={result} />
                    ))}
                </ul>
            </div>
          )}
