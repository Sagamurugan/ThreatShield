...
function ScanResult({ result, fileName }) {
  ...
  if (result.error)
    return <div className="widget-result-box error">{result.error}</div>;
  ...
  if (result.risk_score !== undefined) {
    ...
    return (
      <div
        className={`widget-result-box ${isMalicious ? "malicious" : "safe"}`}
      >
...
function DashboardPage() {
  ...
  useEffect(() => {
    ...
    setOverallRiskScore((prevScore) =>
      Math.max(0, Math.min(100, prevScore + scoreChange))
    );
  }, [urlScanResult]);
  ...
  const handleFileScan = async (file, scanType) => {
    ...
    if (scanType === "cloud") {
      ...
      apiUrl = `${import.meta.env.VITE_API_BASE_URL}/api/scan/cloud-config`;
    } else if (scanType === "ai") {
      setIsAiLoading(true);
      setAiFileName(file.name);
      setAiScanResult(null);
      formData.append("datasetFile", file);
      apiUrl = `${import.meta.env.VITE_API_BASE_URL}/api/scan/ai-dataset`;
    }
      ...
      <div className="main-content">
        ...
        <main className="dashboard-grid">
          <div className="widget large-widget">
            ...
            <div className="risk-score-dial">
              <CircularProgressbar
                ...
                styles={buildStyles({
                  textColor: "#e5e5e5",
                  pathColor: `rgb(${overallRiskScore * 2.55}, ${
                    255 - overallRiskScore * 2.55
                  }, 0)`,
            ...
            <p className="widget-description">
              This score represents the current threat level across all
              monitored assets.
            </p>
          </div>
          ...
          <div className="widget">
            ...
            <p className="widget-description">
              Analyze suspicious URLs in real-time.
            </p>
          ...
          <div className="widget">
            ...
            <p className="widget-description">
              Upload a config file to start the scan.
            </p>
            <div className="widget-content">
              <label
                className={`widget-button file-button ${
                  isCloudLoading ? "disabled" : ""
                }`}
              ...
              >
                {isCloudLoading ? (
                  ...
                  cloudFileName || "Upload & Scan Config"
                )}
              </label>
              <input
                id="cloud-file-input"
                type="file"
                className="widget-file-input"
                onChange={(e) => handleFileScan(e.target.files[0], "cloud")}
          ...
          <div className="widget">
            ...
            <p className="widget-description">
              Upload a dataset to start the scan.
            </p>
            <div className="widget-content">
              <label
                className={`widget-button file-button ${
              ...
              >
                {isAiLoading ? (
                  ...
                  aiFileName || "Upload & Scan Dataset"
                )}
              </label>
              <input
                id="ai-file-input"
                ...
                accept=".csv"
                className="widget-file-input"
                onChange={(e) => handleFileScan(e.target.files[0], "ai")}
